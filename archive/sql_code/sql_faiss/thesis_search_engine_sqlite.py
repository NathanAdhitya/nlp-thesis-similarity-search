...
class ThesisSimilaritySearch:
    ...
    def load_index(self, cursor, limit=None):
        ...
        if self.use_abstract and not self.use_title:
            ...
            embedding_column = f"abstract_embeddings_{self.model_name[22:]}"
            print(f"Using abstract embeddings ({embedding_column[22:]})")
        elif self.use_title and not self.use_abstract:
            # Remove 'sentence-transformers/' prefix
            embedding_column = f"title_embeddings_{self.model_name[22:]}"
        ...
        if not papers:
            raise ValueError(
                f"No papers found with {embedding_column} not null")
    ...
    def _fetch_authors(self, cursor):
        ...
        papers_with_authors = sum(
            1 for pid in self.paper_ids if self.paper_metadata[pid]['authors'])
        print(
            f"Found authors for {papers_with_authors} out of {len(self.paper_ids)} papers")
    ...
    def _fetch_contributors(self, cursor):
        ...
        papers_with_contributors = sum(
            1 for pid in self.paper_ids if self.paper_metadata[pid]['contributors'])
        print(
            f"Found contributors for {papers_with_contributors} out of {len(self.paper_ids)} papers")
    ...
    def search(self, query_text: str, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        query_embedding = np.array(get_embedding(
            query_text, self.model)).reshape(1, -1).astype('float32')
...
class SearchEngine:
    ...
    def load(self, use_title=True, use_abstract=False, limit=None, force_reload=False):
        ...
        search_engine = ThesisSimilaritySearch(
            model=self.model, use_title=use_title, use_abstract=use_abstract)
    ...
    def search_by_people_and_topic(self, query, author_name=None, contributor_name=None,
        ...
        if author_name:
            ...
            results = [r for r in results if any(
                author_name in author.lower() for author in r['authors'])]
            ...
            for r in results:
                if 'contributors' in r and r['contributors']:
                    for contributor in r['contributors']:
                        ...
                        if contributor_name:
                            name_match = contributor_name.lower(
                            ) in contributor['name'].lower()

                        if contributor_role:
                            role_match = contributor_role.lower(
                            ) in contributor['role'].lower()
