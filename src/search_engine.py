...
class SearchEngine:
    def __init__(self, device: Optional[str] = None) -> None:
        """
        Initialize the SearchEngine with embedding models.
        
        Args:
            device: Device to run models on ('cuda' or 'cpu'). 
                   If None, uses configuration default (auto-detect).
        """
        # Determine device
        if device is None:
        ...
        # Load BGEM3 with explicit device using config
        self.bgem3 = BGEM3FlagModel(MODELS["bgem3"], use_fp16=True, device=device)
        
        # Load other models concurrently
        with concurrent.futures.ThreadPoolExecutor() as executor:
            allminilm_future = executor.submit(
                self._load_sentence_transformer, 
                MODELS["allminilm"]
            )
            indobert_future = executor.submit(
                self._load_sentence_transformer, 
                MODELS["indobert"]
            )
            translator_future = executor.submit(
                GoogleTranslator, 
                source='auto', 
                target='indonesian'
            )
    ...
    def _load_sentence_transformer(self, model_name: str) -> SentenceTransformer:
        """Load a SentenceTransformer model on the configured device."""
    ...
    def search_advisor_3(self, query: str, top_k: int = 10, option: str = "bgem3", count_weight: float = 0.4, program_ids: Optional[List[int]] = None) -> List[Dict[str, Any]]:
        """
        Searches for advisors/users with expertise related to the provided query,
    ...
    def _encode_query(self, query: str, model: Union[BGEM3FlagModel, SentenceTransformer]) -> List[float]:
        """
        Encodes a query string using the specified model.
        
        Args:
            query (str): The query string to encode.
            model: The model to use for encoding (BGEM3FlagModel or SentenceTransformer).
    ...
    def _search(self, query_embedding: List[float], top_k: int = 5, option: str = "bgem3") -> List[Dict[str, Any]]:
        ...
        # Use vector table names from config
        if option not in VECTOR_TABLES:
            raise ValueError(f"Invalid model type: {option}. Choose from {list(VECTOR_TABLES.keys())}.")
        
        table_name = VECTOR_TABLES[option]
