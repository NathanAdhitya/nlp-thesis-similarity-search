...
try:
    from data_pipeline.processing.path_utils import get_authors_csv_file, get_canonical_scholar_file
    from data_pipeline.processing.step_01_dewey_cleanup import (
        standardize_name,
        weighted_name_distance,
        cluster_names_by_similarity,
    )
except ImportError:
    from path_utils import get_authors_csv_file, get_canonical_scholar_file
    from step_01_dewey_cleanup import (
        standardize_name,
        weighted_name_distance,
        cluster_names_by_similarity,
    )
...
def load_scholar_authors(csv_file: str) -> Set[str]:
    ...
    print(
        f"Successfully loaded {len(author_names)} unique scholar author names")
    return author_names
...
def clean_scholar_name(name: str) -> str:
    ...
    name = re.sub(r'\b(?:Prof\.?|Dr\.?|PhD\.?|Ph\.D\.?|M\.D\.?|Professor|Dr)\b',
...
def analyze_scholar_authors(csv_file: str, max_distance: float = 2.0, save_canonical: bool = True):
    ...
    standardized_names = {standardize_name(
        name) for name in author_names if standardize_name(name)}
    ...
    if save_canonical:
        output_file = str(get_canonical_scholar_file())
    ...
    print(
        f"Reduction: {len(merged_names)} names ({len(merged_names)/len(standardized_names)*100:.1f}%)")
...
if __name__ == "__main__":
    try:
        from data_pipeline.processing.path_utils import get_canonical_scholar_file, get_authors_csv_file
    except ImportError:
        from path_utils import get_canonical_scholar_file, get_authors_csv_file
    import os
    ...
    csv_file = str(get_authors_csv_file())
