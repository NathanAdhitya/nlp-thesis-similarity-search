...
try:
    from data_pipeline.processing.step_01_dewey_cleanup import weighted_name_distance
except ImportError:
    from step_01_dewey_cleanup import weighted_name_distance
...
def combine_author_datasets(dewey_file: str, scholar_file: str, output_file: str, cross_match_threshold: float = 2.0):
    ...
    dewey_mapping = dewey_data.get('canonical_mapping', {})
    scholar_mapping = scholar_data.get('canonical_mapping', {})
    
    print(f"Loaded Dewey data: {len(dewey_mapping)} mappings")
    print(f"Loaded Scholar data: {len(scholar_mapping)} mappings")

    # Get all canonical names from both datasets
    dewey_canonicals = set(dewey_mapping.values())
    scholar_canonicals = set(scholar_mapping.values())
    ...
    for match in cross_matches:
        ...
        if dewey_canonical not in matched_dewey and scholar_canonical not in matched_scholar:
            ...
            dewey_variants = [name for name, canonical in dewey_mapping.items()
                              if canonical == dewey_canonical]
            scholar_variants = [name for name, canonical in scholar_mapping.items()
    ...
    # Create output data
    # Count unique canonical names (already calculated above)
    dewey_canonical_count = len(dewey_canonicals)
    scholar_canonical_count = len(scholar_canonicals)
    ...
    output_data = {
        'metadata': {
            ...
            'dewey_clusters': dewey_canonical_count,
            'scholar_clusters': scholar_canonical_count,
...
if __name__ == "__main__":
    try:
        from data_pipeline.processing.path_utils import get_canonical_dewey_file, get_canonical_scholar_file, get_combined_authors_file
    except ImportError:
        from path_utils import get_canonical_dewey_file, get_canonical_scholar_file, get_combined_authors_file
    import os
    
    dewey_file = str(get_canonical_dewey_file())
    scholar_file = str(get_canonical_scholar_file())
    output_file = str(get_combined_authors_file())
    
    # Skip if output already exists
    if os.path.exists(output_file):
        print(f"[SKIP] Output file already exists: {output_file}")
        print("Delete the file to regenerate or run processing manually.")
        exit(0)

    combine_author_datasets(dewey_file, scholar_file,
                            output_file, cross_match_threshold=2.0)
