...
try:
    from data_pipeline.processing.step_01_dewey_cleanup import standardize_name, extract_contributor_info, is_advisor_role
    from data_pipeline.processing.path_utils import get_dewey_thesis_dir, get_cleaned_dewey_file, get_publications_dir, get_cleaned_publications_file, get_merged_authors_file
except ImportError:
    from step_01_dewey_cleanup import standardize_name, extract_contributor_info, is_advisor_role
    from path_utils import get_dewey_thesis_dir, get_cleaned_dewey_file, get_publications_dir, get_cleaned_publications_file, get_merged_authors_file
    ...
    with open(csv_file, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            original_names = row.get('original_names', '').split(
                '; ') if row.get('original_names') else []
...
def extract_dewey_thesis_data(data_dir: str, name_to_dewey_id: Dict, output_file: str):
                ...
                if contributors_text:
                    ...
                    for contributor in contributors:
                        if is_advisor_role(contributor['role']):
                            ...
                            if clean_name:
                                standardized_name = standardize_name(
                                    clean_name)
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as f:
        fieldnames = ['dewey_thesis_id',
                      'dewey_ids', 'year', 'title', 'abstract']
...
def extract_publication_data(data_dir: str, name_to_scholar_id: Dict, output_file: str):
                ...
                for line_num, line in enumerate(f, 1):
                    try:
                        ...
                        if author_string:
                            ...
                            authors = [author.strip() for author in re.split(
                                r'\s+and\s+', author_string)]
    ...
    print(
        f"Extracted {len(publication_data)} publication records to {output_file}")


if __name__ == "__main__":
    try:
        from data_pipeline.processing.path_utils import get_cleaned_dewey_file, get_cleaned_publications_file, get_merged_authors_file
    except ImportError:
        from path_utils import get_cleaned_dewey_file, get_cleaned_publications_file, get_merged_authors_file
    import os
    
    # Skip if outputs already exist
    dewey_output = str(get_cleaned_dewey_file())
    pubs_output = str(get_cleaned_publications_file())
    if os.path.exists(dewey_output) and os.path.exists(pubs_output):
        print(f"[SKIP] Output files already exist:")
        print(f"  - {dewey_output}")
        print(f"  - {pubs_output}")
        print("Delete the files to regenerate or run processing manually.")
        exit(0)
    
    # Load merged authors for ID mapping
    name_to_dewey_id, name_to_scholar_id = load_merged_authors(
        str(get_merged_authors_file()))
    ...
    # Extract thesis data
    extract_dewey_thesis_data(str(get_dewey_thesis_dir()),
                              name_to_dewey_id, str(get_cleaned_dewey_file()))

    # Extract publication data
    extract_publication_data(
        str(get_publications_dir()), name_to_scholar_id, str(get_cleaned_publications_file()))
