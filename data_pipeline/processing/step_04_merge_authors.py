...
try:
    from data_pipeline.processing.step_01_dewey_cleanup import standardize_name
except ImportError:
    from step_01_dewey_cleanup import standardize_name
...
def merge_authors(scholar_authors: Dict, combined_mapping: Dict, output_file: str):
    ...
    for name in sorted(all_names):
        ...
        if canonical_name not in merged_authors:
            ...
            scholar_info = scholar_authors.get(
                name) or scholar_authors.get(canonical_name)
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as f:
        ...
        for author_data in merged_authors.values():
            ...
            author_data['original_names'] = '; '.join(
                author_data['original_names'])
...
if __name__ == "__main__":
    # Load data
    try:
        from data_pipeline.processing.path_utils import get_authors_csv_file, get_combined_authors_file, get_merged_authors_file
    except ImportError:
        from path_utils import get_authors_csv_file, get_combined_authors_file, get_merged_authors_file
    import os
    
    # Skip if output already exists
    output_file = str(get_merged_authors_file())
    if os.path.exists(output_file):
        print(f"[SKIP] Output file already exists: {output_file}")
        print("Delete the file to regenerate or run processing manually.")
        exit(0)
    
    scholar_authors = load_scholar_authors(str(get_authors_csv_file()))
    combined_mapping = load_combined_authors(str(get_combined_authors_file()))

    # Merge authors
    merged_authors = merge_authors(
        scholar_authors, combined_mapping, str(get_merged_authors_file()))
